    ...
    for inp in sorted(glob.glob(os.path.join(options.fcd, "*.fcd.xml"))):
        ...
        if not options.modes or mode in options.modes.split(","):
            ...
            if options.network_split_vclass:
                ...
                # TODO: Check if we have some trolleybus-specific edge types in OSM/SUMO; probably not
                elif mode == "trolleybus":
                    # TODO: Netconvert probably does not know about "trolleybus" mode
                    # Trolleybuses should be handled like buses
                    edgeFilter = ["--keep-edges.by-vclass", "bus"]
